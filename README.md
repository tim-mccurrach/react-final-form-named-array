...
In order for `update` and `remove` to be

### Usage

The `FieldArray` object provided by `react-final-form-named-arrays` has all of the same props as the normal `FieldArray` from `react-final-form-arrays` however it has one additional optional prop, `getItemName`. If this prop is ommited, the behaviour of the component will be the same as the original. If included, it adds the ability to use the modifiers `update`, `remove`, `removeBatch`, `swap`, and `move`

```
<FieldObject
  name="myField"
  getIndexName=(Any)=>String
  render={({fields})=>fields.map(x=>
    <Component {...x} />
    )}
  />
```

# Note about how to use these modifiers outside of the FieldArray

# Roadmap

1. Add all of the mutators (concat, insert, removeBatch, shift, unshift, swap)
2. Once issues in the main repos regarding `modified` are resolved, update the way initial names are processed
3. See if there is want for this to be added to the main `react-final-form-arrays` repo - it may well be to niche, and be considered unnecessary bloat.

YOU NEED TO SUBSCRIBE TO data, pristine and initial
